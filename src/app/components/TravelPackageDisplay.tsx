...
import { CheckCircle, ListCheck, ListXIcon, Rocket, Sparkles, Star, User, X } from "lucide-react";
...
const TravelPackageDisplay = ({ travelPackage }: Props) => {
    ...
    useEffect(() => {
        const fetchPackageData = async () => {
            if (!id || !type) return; // Ensure both exist
            try {
                const response = await fetch(`/api/packages/${type}/${id}`);
    ...
    const handleBookNow = () => {
        router.push(`/PackageLead/${type}/${id}`); // Redirect to Package Lead form with type and id
    };
